...
def print_wrench_version(ctx, param, value):
	"""Prints current wrench version"""
	if not value or ctx.resilient_parsing:
		return

	import wrench

	click.echo(wrench.VERSION)
	ctx.exit()
...
def use_experimental_feature(ctx, param, value):
	...
	if value == "dynamic-feed":
		...
		wrench.cli.verbose = True
	else:
		from wrench.exceptions import FeatureDoesNotExistError

		raise FeatureDoesNotExistError(f"Feature {value} does not exist")

	from wrench.cli import is_envvar_warn_set

	if is_envvar_warn_set:
		return

	click.secho(
		"WARNING: wrench is using it's new CLI rendering engine. This behaviour has"
