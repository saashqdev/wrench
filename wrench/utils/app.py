...
from wrench.exceptions import (
	InvalidRemoteException,
	InvalidBranchException,
	CommandFailedError,
	VersionNotFound,
)
from wrench.app import get_repo_dir


def is_version_upgrade(app="saashq", wrench_path=".", branch=None):
	upstream_version = get_upstream_version(app=app, branch=branch, wrench_path=wrench_path)
	...
	local_version = get_major_version(get_current_version(app, wrench_path=wrench_path))
...
def switch_branch(branch, apps=None, wrench_path=".", upgrade=False, check_upgrade=True):
	import git
	from wrench.wrench import Wrench
	from wrench.utils import log, exec_cmd
	...
	for app in apps:
		...
		if check_upgrade:
			version_upgrade = is_version_upgrade(app=app, wrench_path=wrench_path, branch=branch)
	...
	if switched_apps:
		...
		print(
			"Please run `wrench update --patch` to be safe from any differences in"
			" database schema"
		)

	if version_upgrade[0] and upgrade:
		Wrench(wrench_path).setup.requirements()
		backup_all_sites()
		patch_sites()
		build_assets()
...
def switch_to_branch(branch=None, apps=None, wrench_path=".", upgrade=False):
	switch_branch(branch, apps=apps, wrench_path=wrench_path, upgrade=upgrade)


def switch_to_develop(apps=None, wrench_path=".", upgrade=True):
	switch_branch("develop", apps=apps, wrench_path=wrench_path, upgrade=upgrade)
...
def get_develop_version(app, wrench_path="."):
	repo_dir = get_repo_dir(app, wrench_path=wrench_path)
...
def get_upstream_version(app, branch=None, wrench_path="."):
	repo_dir = get_repo_dir(app, wrench_path=wrench_path)
	if not branch:
...
def get_current_saashq_version(wrench_path="."):
	try:
		return get_major_version(get_current_version("saashq", wrench_path=wrench_path))
	except OSError:
		return 0


def get_current_branch(app, wrench_path="."):
...
def get_remote(app, wrench_path="."):
	repo_dir = get_repo_dir(app, wrench_path=wrench_path)
	contents = subprocess.check_output(
...
def get_app_name(wrench_path: str, folder_name: str) -> str:
	...
	"""
	app_name = None
	apps_path = os.path.join(os.path.abspath(wrench_path), "apps")
...
def check_existing_dir(wrench_path, repo_name):
	cloned_path = os.path.join(wrench_path, "apps", repo_name)
	dir_already_exists = os.path.isdir(cloned_path)
	return dir_already_exists, cloned_path


def get_current_version(app, wrench_path="."):
