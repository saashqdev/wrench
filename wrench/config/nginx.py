...
from wrench.utils import get_wrench_name


def make_nginx_conf(wrench_path, yes=False, logging=None, log_format=None):
	conf_path = os.path.join(wrench_path, "config", "nginx.conf")
	...
	if allow_rate_limiting:
		template_vars.update(
			{
				"wrench_name_hash": hashlib.sha256(wrench_name).hexdigest()[:16],
...
def make_wrench_manager_nginx_conf(wrench_path, yes=False, port=23624, domain=None):
	from wrench.config.site_config import get_site_config

	template = wrench.config.env().get_template("wrench_manager_nginx.conf")
	wrench_path = os.path.abspath(wrench_path)
	sites_path = os.path.join(wrench_path, "sites")

	config = Wrench(wrench_path).conf
	site_config = get_site_config(domain, wrench_path=wrench_path)
	wrench_name = get_wrench_name(wrench_path)

	template_vars = {
		"port": port,
		"domain": domain,
		"wrench_manager_site_name": "wrench-manager.local",
		"sites_path": sites_path,
		"http_timeout": config.get("http_timeout"),
		"webserver_port": config.get("webserver_port"),
		"socketio_port": config.get("socketio_port"),
	...
	wrench_manager_nginx_conf = template.render(**template_vars)

	conf_path = os.path.join(wrench_path, "config", "nginx.conf")

	if not yes and os.path.exists(conf_path):
		click.confirm(
			"nginx.conf already exists and wrench-manager configuration will be appended to it. Do you want to continue?",
			abort=True,
		)

	with open(conf_path, "a") as myfile:
		myfile.write(wrench_manager_nginx_conf)


def prepare_sites(config, wrench_path):
	sites = {
		"that_use_port": [],
		"that_use_dns": [],
		"that_use_ssl": [],
	...
	sites_configs = get_sites_with_config(wrench_path=wrench_path)
...
def get_sites_with_config(wrench_path):
	from wrench.wrench import Wrench
	from wrench.config.site_config import get_site_config
	...
	for site in sites:
		try:
			site_config = get_site_config(site, wrench_path=wrench_path)
	...
	use_wildcard_certificate(wrench_path, ret)

	return ret


def use_wildcard_certificate(wrench_path, ret):
	"""
	stored in common_site_config.json as:
	"wildcard": {
	        "domain": "*.erpnexus.com",
	        "ssl_certificate": "/path/to/erpnexus.com.cert",
	        "ssl_certificate_key": "/path/to/erpnexus.com.key"
	}
	"""
	from wrench.wrench import Wrench
	...
	for site in ret:
		...
		if (site.get("domain") or site["name"]).endswith(domain):
			# example: ends with .erpnexus.com
...
def get_error_pages():
	wrench_app_path = os.path.abspath(wrench.__path__[0])
	templates = os.path.join(wrench_app_path, "config", "templates")
