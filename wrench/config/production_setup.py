...
from wrench.utils import exec_cmd, which, get_wrench_name, get_cmd_output, log
from wrench.utils.system import fix_prod_setup_perms
...
def setup_production_prerequisites():
	...
	if not which("fail2ban-client"):
		exec_cmd("wrench setup role fail2ban")
	if not which("nginx"):
		exec_cmd("wrench setup role nginx")
	if not which("supervisord"):
		exec_cmd("wrench setup role supervisor")


def setup_production(user, wrench_path=".", yes=False):
	...
	if conf.get("restart_systemd_on_update"):
		...
		generate_systemd_config(wrench_path=wrench_path, user=user, yes=yes)
	else:
		print("Setting Up supervisor...")
		check_supervisord_config(user=user)
		generate_supervisor_config(wrench_path=wrench_path, user=user, yes=yes)

	print("Setting Up NGINX...")
	make_nginx_conf(wrench_path=wrench_path, yes=yes)
	fix_prod_setup_perms(wrench_path, saashq_user=user)
	...
	if conf.get("restart_supervisor_on_update"):
		...
		supervisor_conf = os.path.join(
			get_supervisor_confdir(), f"{wrench_name}.{supervisor_conf_extn}"
		)

		# Check if symlink exists, If not then create it.
		if not os.path.islink(supervisor_conf):
			os.symlink(
				os.path.abspath(os.path.join(wrench_path, "config", "supervisor.conf")),
				supervisor_conf,
			)

	if not os.path.islink(nginx_conf):
		os.symlink(
			os.path.abspath(os.path.join(wrench_path, "config", "nginx.conf")), nginx_conf
		)
...
def disable_production(wrench_path="."):
	wrench_name = get_wrench_name(wrench_path)
	conf = Wrench(wrench_path).conf
...
def service(service_name, service_option):
	if os.path.basename(which("systemctl") or "") == "systemctl" and is_running_systemd():
		...
		service_manager = os.environ.get("WRENCH_SERVICE_MANAGER")
		if service_manager:
			service_manager_command = (
				os.environ.get("WRENCH_SERVICE_MANAGER_COMMAND")
