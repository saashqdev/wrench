...
from wrench.utils import exec_cmd, run_playbook, which
from wrench.utils.cli import SugaredOption


@click.group(help="Setup command group for enabling setting up a Saashq environment")
def setup():
	pass
...
def setup_sudoers(user):
	from wrench.utils.system import setup_sudoers
...
def setup_nginx(yes=False, logging="combined", log_format=None):
	...
	make_nginx_conf(wrench_path=".", yes=yes, logging=logging, log_format=log_format)


@click.command("reload-nginx", help="Checks NGINX config file and reloads service")
def reload_nginx():
	from wrench.config.production_setup import reload_nginx
...
def setup_supervisor(user=None, yes=False, skip_redis=False, skip_supervisord=False):
	from wrench.utils import get_cmd_output
	from wrench.config.supervisor import (
	...
	generate_supervisor_config(wrench_path=".", user=user, yes=yes, skip_redis=skip_redis)
...
@click.command("fonts", help="Add Saashq fonts to system")
def setup_fonts():
	from wrench.utils.system import setup_fonts

	setup_fonts()


@click.command(
	"production", help="Setup Saashq production environment for specific user"
)
@click.argument("user")
@click.option("--yes", help="Yes to regeneration config", is_flag=True, default=False)
def setup_production(user, yes=False):
	from wrench.config.production_setup import setup_production

	setup_production(user=user, yes=yes)


@click.command("backups", help="Add cronjob for wrench backups")
def setup_backups():
...
@click.command("env", help="Setup Python environment for wrench")
@click.option(
...
def setup_env(python="python3"):
	...
	return Wrench(".").setup.env(python=python)
...
def setup_firewall(ssh_port=None, force=False):
	...
	run_playbook("roles/wrench/tasks/setup_firewall.yml", {"ssh_port": ssh_port})
...
def set_ssh_port(port, force=False):
	...
	run_playbook("roles/wrench/tasks/change_ssh_port.yml", {"ssh_port": port})
...
def setup_letsencrypt(site, custom_domain, non_interactive):
	from wrench.config.lets_encrypt import setup_letsencrypt

	setup_letsencrypt(site, custom_domain, wrench_path=".", interactive=not non_interactive)


@click.command(
	"wildcard-ssl", help="Setup wildcard SSL certificate for multi-tenant wrench"
)
...
def setup_wildcard_ssl(domain, email, exclude_base_domain):
	from wrench.config.lets_encrypt import setup_wildcard_ssl

	setup_wildcard_ssl(
		domain, email, wrench_path=".", exclude_base_domain=exclude_base_domain
	)


@click.command("procfile", help="Generate Procfile for wrench start")
def setup_procfile():
...
def setup_requirements(node=False, python=False, dev=False, apps=None):
	...
	if not (node or python or dev):
		wrench.setup.requirements(apps=apps)

	elif not node and not dev:
		wrench.setup.python(apps=apps)

	elif not python and not dev:
		wrench.setup.node(apps=apps)

	else:
		from wrench.utils.wrench import install_python_dev_dependencies
...
@click.command(
	"manager",
	help="Setup wrench-manager.local site with the wrench_manager app installed on it",
)
@click.option(
	"--yes", help="Yes to regeneration of nginx config file", default=False, is_flag=True
)
@click.option(
	"--port", help="Port on which you want to run wrench manager", default=23624
)
@click.option("--domain", help="Domain on which you want to run wrench manager")
def setup_manager(yes=False, port=23624, domain=None):
	from wrench.wrench import Wrench
	from wrench.config.nginx import make_wrench_manager_nginx_conf

	create_new_site = True

	if "wrench-manager.local" in os.listdir("sites"):
		create_new_site = click.confirm("Site already exists. Overwrite existing site?")

	if create_new_site:
		exec_cmd("wrench new-site --force wrench-manager.local")

	if "wrench_manager" in os.listdir("apps"):
		print("App already exists. Skipping app download.")
	else:
		exec_cmd("wrench get-app wrench_manager")

	exec_cmd("wrench --site wrench-manager.local install-app wrench_manager")

	wrench_path = "."
	wrench = Wrench(wrench_path)

	if wrench.conf.get("restart_supervisor_on_update") or wrench.conf.get(
		"restart_systemd_on_update"
	):
		# implicates a production setup or so I presume
		if not domain:
			print(
				"Please specify the site name on which you want to host wrench-manager using the 'domain' flag"
			)
			sys.exit(1)

		if domain not in wrench.sites:
			raise Exception("No such site")

		make_wrench_manager_nginx_conf(wrench_path, yes=yes, port=port, domain=domain)
...
def add_domain(domain, site=None, ssl_certificate=None, ssl_certificate_key=None):
	...
	from wrench.config.site_config import add_domain

	add_domain(site, domain, ssl_certificate, ssl_certificate_key, wrench_path=".")
...
def remove_domain(domain, site=None):
	...
	from wrench.config.site_config import remove_domain

	remove_domain(site, domain, wrench_path=".")
...
def sync_domains(domain=None, site=None):
	...
	from wrench.config.site_config import sync_domains

	changed = sync_domains(site, domains, wrench_path=".")
...
@click.option("--stop", help="Stop wrench services", is_flag=True, default=False)
...
def setup_systemd(
	...
	from wrench.config.systemd import generate_systemd_config
