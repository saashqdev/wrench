...
from wrench.utils.wrench import post_upgrade, patch_sites, build_assets


@click.command(
	"update",
	help="Performs an update operation on current wrench. Without any flags will backup, pull, setup requirements, build, run patches and restart wrench. Using specific flags will only do certain tasks instead of all",
)
@click.option("--pull", is_flag=True, help="Pull updates for all the apps in wrench")
@click.option("--apps", type=str)
@click.option("--patch", is_flag=True, help="Run migrations for all sites in the wrench")
@click.option("--build", is_flag=True, help="Build JS and CSS assets for the wrench")
@click.option(
	"--requirements",
	is_flag=True,
	help="Update requirements. If run alone, equivalent to `wrench setup requirements`",
)
@click.option(
...
def update(
	...
	from wrench.utils.wrench import update
...
def switch_to_branch(branch, apps, upgrade=False):
	from wrench.utils.app import switch_to_branch
...
def switch_to_develop(upgrade=False):
	"Switch saashq and erpnexus to develop branch"
	from wrench.utils.app import switch_to_develop

	switch_to_develop(apps=["saashq", "erpnexus"])
