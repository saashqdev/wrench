...
from wrench.config.common_site_config import update_config, put_config

# imports - third party imports
import click


@click.group(help="Change wrench configuration")
def config():
	pass
...
@click.command(
	"dns_multitenant", help="Enable/Disable wrench multitenancy on running wrench update"
)
@click.argument("state", type=click.Choice(["on", "off"]))
...
def set_common_config(configs):
	...
	update_config(common_site_config, wrench_path=".")


@click.command(
	"remove-common-config", help="Remove specific keys from current wrench's common config"
)
@click.argument("keys", nargs=-1)
def remove_common_config(keys):
	from wrench.wrench import Wrench

	common_site_config = Wrench(".").conf
	for key in keys:
