...
import wrench
from wrench.exceptions import NotInWrenchDirectoryError
...
from wrench.utils.wrench import build_assets, install_python_dev_dependencies
from wrench.utils.render import step
...
class AppMeta:
	def __init__(self, name: str, branch: str = None, to_clone: bool = True):
		...
		class Healthcare(AppConfig):
		        dependencies = [{"saashq/erpnexus": "~13.17.0"}]
		"""
	...
	def setup_details(self):
		...
		if self.wrench and os.path.exists(os.path.join(self.wrench.name, "apps", self.name)):
			self.mount_path = os.path.join(self.wrench.name, "apps", self.name)
			self.from_apps = True
...
class App(AppMeta):
	...
	def get(self):
		...
		shallow = "--depth 1" if self.wrench.shallow_clone else ""
		...
		self.wrench.run(
			f"{cmd} {args}",
			cwd=os.path.join(self.wrench.name, "apps"),
		)
	...
	def install(
		...
		import wrench.cli
		from wrench.utils.app import get_app_name

		self.validate_app_dependencies()

		verbose = wrench.cli.verbose or verbose
		app_name = get_app_name(self.wrench.name, self.app_name)
		if not resolved and self.app_name != "saashq" and not ignore_resolution:
			click.secho(
		...
		install_app(
			app=app_name,
			tag=self.tag,
			wrench_path=self.wrench.name,
			verbose=verbose,
			skip_assets=skip_assets,
			restart_wrench=restart_wrench,
	...
	def uninstall(self):
		self.wrench.run(f"{self.wrench.python} -m pip uninstall -y {self.name}")

	def _get_dependencies(self):
		from wrench.utils.app import get_required_deps, required_apps_from_hooks
	...
	def update_app_state(self):
		...
		wrench = Wrench(self.wrench.name)
		wrench.apps.sync(
	...
	def get_pyproject(self) -> Optional[dict]:
		from wrench.utils.app import get_pyproject

		if self.pyproject:
			return self.pyproject

		apps_path = os.path.join(os.path.abspath(self.wrench.name), "apps")
	...
	def validate_app_dependencies(self, throw=False) -> None:
		...
		deps: Optional[dict] = (
			pyproject.get("tool", {}).get("wrench", {}).get("saashq-dependencies")
		)
		if not deps:
			return
	...
	wrench sub directories. If we assume deterministic builds
	...
	def get_app_path(self) -> Path:
		return Path(self.wrench.name) / "apps" / self.app_name
	...
	def get_cached(self) -> bool:
		...
		click.secho(
			f"Wrench app-cache: extracting {self.app_name} from {cache_path.as_posix()}",
		)
		...
		with tarfile.open(cache_path, mode) as tar:
			...
			try:
				...
				click.secho(
					f"Wrench app-cache: extraction succeeded for {self.app_name}",
					fg="green",
				)
			except Exception:
				message = f"Wrench app-cache: extraction failed for {self.app_name}"
				click.secho(
					message,
					fg="yellow",
				)
	...
	def set_cache(self, compress_artifacts=False) -> bool:
		...
		message = f"Wrench app-cache: caching {self.app_name}"
		...
		try:
			...
			click.secho(
				f"Wrench app-cache: caching succeeded for {self.app_name} as {hashed_path.as_posix()}",
				fg="green",
			)

			success = True
		except Exception as exc:
			log(f"Wrench app-cache: caching failed for {self.app_name} {exc}", level=3)
			success = False
		finally:
			os.chdir(cwd)
		return success
...
def can_saashq_use_cached(app: App) -> bool:
	min_saashq = get_required_saashq_version(app)
	if not min_saashq:
		return False

	try:
		return sv.Version(min_saashq) in sv.SimpleSpec(">=15.12.0")
	except ValueError:
	...
	try:
		...
		"""
		return sv.Version("15.12.0") not in sv.SimpleSpec(min_saashq)
	except ValueError:
		click.secho(
			f"Wrench app-cache: invalid value found for saashq version '{min_saashq}'",
			fg="yellow",
		)
		# Invalid expression
		return False
...
def validate_dependency(app: App, dep: str, req_version: str, throw=False) -> None:
	dep_path = Path(app.wrench.name) / "apps" / dep
	if not dep_path.is_dir():
		click.secho(f"Required saashq-dependency '{dep}' not found.", fg="yellow")
		if throw:
			sys.exit(1)
		return
	...
	if sv.Version(dep_version) not in sv.SimpleSpec(req_version):
		click.secho(
			f"Installed saashq-dependency '{dep}' version '{dep_version}' "
...
def get_required_saashq_version(app: App) -> Optional[str]:
	pyproject = app.get_pyproject() or {}

	# Reference: https://github.com/saashqdev/wrench/issues/1524
	req_saashq = (
		pyproject.get("tool", {})
		.get("wrench", {})
		.get("saashq-dependencies", {})
		.get("saashq")
	)

	if not req_saashq:
		click.secho(
			"Required saashq version not set in pyproject.toml, "
			"please refer: https://github.com/saashqdev/wrench/issues/1524",
			fg="yellow",
		)
...
def remove_unused_node_modules(app_path: Path) -> None:
	...
	Since most popular Saashq apps use Vite to build their frontends,
	...
	for p in app_path.iterdir():
		...
		if can_delete:
			click.secho(
				f"Wrench app-cache: removing {node_modules.as_posix()}",
				fg="yellow",
			)
			shutil.rmtree(node_modules)


def make_resolution_plan(app: App, wrench: "Wrench"):
	"""
	...
	for app_name in app._get_dependencies():
		dep_app = App(app_name, wrench=wrench)
		is_valid_saashq_branch(dep_app.url, dep_app.branch)
		...
		resolution.update(make_resolution_plan(dep_app, wrench))
		app.local_resolution = [repo_name for repo_name, _ in reversed(resolution.items())]
	return resolution


def get_excluded_apps(wrench_path="."):
	try:
		with open(os.path.join(wrench_path, "sites", "excluded_apps.txt")) as f:
			return f.read().strip().split("\n")
	except OSError:
		return []


def add_to_excluded_apps_txt(app, wrench_path="."):
	if app == "saashq":
		raise ValueError("Saashq app cannot be excluded from update")
	if app not in os.listdir("apps"):
		raise ValueError(f"The app {app} does not exist")
	apps = get_excluded_apps(wrench_path=wrench_path)
	if app not in apps:
		apps.append(app)
		return write_excluded_apps_txt(apps, wrench_path=wrench_path)


def write_excluded_apps_txt(apps, wrench_path="."):
	with open(os.path.join(wrench_path, "sites", "excluded_apps.txt"), "w") as f:
		return f.write("\n".join(apps))


def remove_from_excluded_apps_txt(app, wrench_path="."):
	apps = get_excluded_apps(wrench_path=wrench_path)
	if app in apps:
		apps.remove(app)
		return write_excluded_apps_txt(apps, wrench_path=wrench_path)
...
def get_app(
	...
	"""wrench get-app clones a Saashq App from remote (GitHub or any other git server),
	and installs it on the current wrench. This also resolves dependencies based on the
	apps' required_apps defined in the hooks.py file.

	If the wrench_path is not a wrench directory, a new wrench is created named using the
	git_url parameter.
	"""
	import wrench as _wrench
	import wrench.cli as wrench_cli
	from wrench.wrench import Wrench
	from wrench.utils.app import check_existing_dir
	...
	app = App(
		git_url, branch=branch, wrench=wrench, soft_link=soft_link, cache_key=cache_key
	)
	git_url = app.url
	repo_name = app.repo
	branch = app.tag
	wrench_setup = False
	restart_wrench = not init_wrench
	saashq_path, saashq_branch = None, None

	if resolve_deps:
		resolution = make_resolution_plan(app, wrench)
		...
		if "saashq" in resolution:
			# Todo: Make saashq a terminal dependency for all saashq apps.
			saashq_path, saashq_branch = resolution["saashq"].url, resolution["saashq"].tag

	if not is_wrench_directory(wrench_path):
		if not init_wrench:
			raise NotInWrenchDirectoryError(
				f"{os.path.realpath(wrench_path)} is not a valid wrench directory. "
				"Run with --init-wrench if you'd like to create a Wrench too."
			)

		from wrench.utils.system import init

		wrench_path = get_available_folder_name(f"{app.repo}-wrench", wrench_path)
		init(
			path=wrench_path,
			saashq_path=saashq_path,
			saashq_branch=saashq_branch or branch,
		)
		os.chdir(wrench_path)
		wrench_setup = True

	if wrench_setup and wrench_cli.from_command_line and wrench_cli.dynamic_feed:
		_wrench.LOG_BUFFER.append(
			{
	...
	dir_already_exists, cloned_path = check_existing_dir(wrench_path, repo_name)
	...
	if (
		...
		app.install(verbose=verbose, skip_assets=skip_assets, restart_wrench=restart_wrench)
...
def install_resolved_deps(
	...
	if "saashq" in resolution:
		# Terminal dependency
		del resolution["saashq"]

	for repo_name, app in reversed(resolution.items()):
		existing_dir, path_to_app = check_existing_dir(wrench_path, repo_name)
		if existing_dir:
			is_compatible = False

			try:
				installed_branch = wrench.apps.states[repo_name]["resolution"]["branch"].strip()
			except Exception:
				installed_branch = (
...
def new_app(app, no_git=None, wrench_path="."):
	if wrench.SAASHQ_VERSION in (0, None):
		raise NotInWrenchDirectoryError(
			f"{os.path.realpath(wrench_path)} is not a valid wrench directory."
		)
	...
	apps = os.path.abspath(os.path.join(wrench_path, "apps"))
	args = ["make-app", apps, app]
	if no_git:
		if wrench.SAASHQ_VERSION < 14:
			click.secho("Saashq v14 or greater is needed for '--no-git' flag", fg="red")
			return
		args.append(no_git)

	logger.log(f"creating new app {app}")
	run_saashq_cmd(*args, wrench_path=wrench_path)
	install_app(app, wrench_path=wrench_path)
...
def install_app(
	...
	app_path = os.path.realpath(os.path.join(wrench_path, "apps", app))

	wrench.run(
		f"{wrench.python} -m pip install {quiet_flag} --upgrade -e {app_path} {cache_flag}"
	)

	if conf.get("developer_mode"):
		install_python_dev_dependencies(apps=app, wrench_path=wrench_path, verbose=verbose)
	...
	wrench.apps.sync(app_name=app, required=resolution, branch=tag, app_dir=app_path)

	if not skip_assets:
		build_assets(wrench_path=wrench_path, app=app, using_cached=using_cached)

	if restart_wrench:
		# Avoiding exceptions here as production might not be set-up
		# OR we might just be generating docker images.
		wrench.reload(_raise=False)


def pull_apps(apps=None, wrench_path=".", reset=False):
	"""Check all apps if there no local changes, pull"""
	from wrench.wrench import Wrench
	from wrench.utils.app import get_current_branch, get_remote

	wrench = Wrench(wrench_path)
	rebase = "--rebase" if wrench.conf.get("rebase_on_pull") else ""
	apps = apps or wrench.apps
	excluded_apps = wrench.excluded_apps
	...
	if not reset:
		for app in apps:
			if app in excluded_apps:
				...
				continue
			app_dir = get_repo_dir(app, wrench_path=wrench_path)
...
2. Temporarily remove your changes with "git stash" or discard them completely
	with "wrench update --reset" or for individual repositries "git reset --hard"
	...
	for app in apps:
		...
		if os.path.exists(os.path.join(app_dir, ".git")):
			...
			if not remote:
				...
				add_to_excluded_apps_txt(app, wrench_path=wrench_path)
				print(
			...
			if not wrench.conf.get("shallow_clone") or not reset:
				...
				if is_shallow:
					...
					wrench.run(f"git fetch {remote} --unshallow", cwd=app_dir)

			branch = get_current_branch(app, wrench_path=wrench_path)
			logger.log(f"pulling {app}")
			if reset:
				reset_cmd = f"git reset --hard {remote}/{branch}"
				if wrench.conf.get("shallow_clone"):
					wrench.run(f"git fetch --depth=1 --no-tags {remote} {branch}", cwd=app_dir)
					wrench.run(reset_cmd, cwd=app_dir)
					wrench.run("git reflog expire --all", cwd=app_dir)
					wrench.run("git gc --prune=all", cwd=app_dir)
				else:
					wrench.run("git fetch --all", cwd=app_dir)
					wrench.run(reset_cmd, cwd=app_dir)
			else:
				wrench.run(f"git pull {rebase} {remote} {branch}", cwd=app_dir)
			wrench.run('find . -name "*.pyc" -delete', cwd=app_dir)


def use_rq(wrench_path):
	wrench_path = os.path.abspath(wrench_path)
	celery_app = os.path.join(wrench_path, "apps", "saashq", "saashq", "celery_app.py")
	return not os.path.exists(celery_app)


def get_repo_dir(app, wrench_path="."):
	return os.path.join(wrench_path, "apps", app)
...
def install_apps_from_path(path, wrench_path="."):
...
def validate_cache_and_get_path(app_name: str, cache_key: str) -> "Optional[Path]":
	...
	if not cache_path.is_file():
		click.secho(
			f"Wrench app-cache: file check failed for {cache_path.as_posix()}, skipping cache",
			fg="yellow",
		)
	...
	if not is_cache_hash_valid(cache_path):
		click.secho(
			f"Wrench app-cache: hash validation failed for {cache_path.as_posix()}, skipping cache",
			fg="yellow",
		)
		unlink_no_throw(cache_path)
		return
