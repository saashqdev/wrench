...
def clone_saashq_docker_repo() -> None:
	try:
		urllib.request.urlretrieve(
			"https://github.com/saashqdev/saashq_docker/archive/refs/heads/main.zip",
			"saashq_docker.zip",
		)
		logging.info("Downloaded saashq_docker zip file from GitHub")
		unpack_archive(
			"saashq_docker.zip", "."
		)  # Unzipping the saashq_docker.zip creates a folder "saashq_docker-main"
		move("saashq_docker-main", "saashq_docker")
		logging.info("Unzipped and Renamed saashq_docker")
		os.remove("saashq_docker.zip")
		logging.info("Removed the downloaded zip file")
	except Exception as e:
		logging.error("Download and unzip failed", exc_info=True)
		cprint("\nCloning saashq_docker Failed\n\n", "[ERROR]: ", e, level=1)
...
def write_to_env(
	...
	admin_pass: str,
	email: str,
	erpnexus_version: str = None,
) -> None:
	quoted_sites = ",".join([f"`{site}`" for site in sites]).strip(",")
	example_env = get_from_env(wd, "example.env")
	erpnexus_version = erpnexus_version or example_env["ERPNEXUS_VERSION"]
	with open(os.path.join(wd, ".env"), "w") as f:
		f.writelines(
			[
				f"ERPNEXUS_VERSION={erpnexus_version}\n",  # defaults to latest version of ERPNexus
...
def check_repo_exists() -> bool:
	return os.path.exists(os.path.join(os.getcwd(), "saashq_docker"))
...
def setup_prod(project: str, sites, email: str, version: str = None, image = None) -> None:
	...
	if check_repo_exists():
		...
		docker_repo_path = os.path.join(os.getcwd(), "saashq_docker")
		cprint(
			"\nPlease refer to .example.env file in the saashq_docker folder to know which keys to set\n\n",
			level=3,
		)
		admin_pass = ""
		db_pass = ""
		with open(compose_file_name, "w") as f:
			...
			try:
				# TODO: Include flags for non-https and non-erpnexus installation
				subprocess.run(
					[
						which("docker"),
						"compose",
	...
	if check_repo_exists():
		try:
			subprocess.run(
				...
				cwd=os.path.join(os.getcwd(), "saashq_docker"),
				check=True,
			)
			cprint(
				"Please go through the Development Documentation: https://github.com/saashqdev/saashq_docker/tree/main/docs/development.md to fully complete the setup.",
				level=2,
			)
...
def create_site(
	...
	try:
		...
		logging.error(f"Wrench site creation failed for {sitename}", exc_info=True)
		cprint(f"Wrench Site creation failed for {sitename}\n", e)
...
if __name__ == "__main__":
	...
	parser.add_argument(
		"-s",
		"--sitename",
		help="Site Name(s) for your production wrench",
		default=[],
		action="append",
		dest="sites",
	)
	parser.add_argument("-n", "--project", help="Project Name", default="saashq")
	parser.add_argument("-i", "--image", help="Full Image Name")
	parser.add_argument(
		"--email", help="Add email for the SSL.", required="--prod" in sys.argv
	)
	parser.add_argument(
		"-v", "--version", help="ERPNexus version to install, defaults to latest stable"
	)
